  ...
  return (
    <>
      <style>{`
        ...
        .back-btn { display:inline-flex; align-items:center; gap:6px; font-size:14px; color:var(--text-muted); text-decoration:none; font-weight:600; transition:color 0.2s; }
      ...
      <main style={{ background:'var(--cream)', minHeight:'100vh', paddingBottom:'80px' }}>
        ...
        <div style={{ background:'white', borderBottom:'1px solid rgba(44,26,14,0.08)', padding:'0 6%', position:'sticky', top:0, zIndex:50 }}>
          <div style={{ maxWidth:'1100px', margin:'0 auto', padding:'16px 0', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            ...
            <a href="/" style={{ fontFamily:'var(--font-hero)', fontSize:'22px', color:'var(--orange)', fontWeight:900, textDecoration:'none' }}>Ninoz</a>
        ...
        <div style={{ padding:'0 6%', maxWidth:'1100px', margin:'0 auto' }}>
          {meals.length === 0 ? (
            <div style={{ textAlign:'center', padding:'60px', color:'var(--text-muted)', fontSize:'15px', background:'white', borderRadius:'20px', border:'1px solid rgba(44,26,14,0.08)' }}>
              No meals added yet.<br/>Add them from admin → Meals.
            </div>
          ) : (
              ...
              {meals.map(meal => (
                <div key={meal.id} className="meal-card" onClick={() => setSelectedMeal(meal)}>
                  ...
                  <div style={{ padding:'14px 16px 18px', flex:1, display:'flex', flexDirection:'column' }}>
                    ...
                    {meal.calories && <div style={{ fontSize:'11px', color:'var(--orange)', fontWeight:600, marginBottom:'8px' }}>{meal.calories}</div>}
                    <div style={{ marginTop:'auto', fontSize:'12px', color:'var(--orange)', fontWeight:700 }}>Tap for details →</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="popup-overlay" onClick={() => setSelectedMeal(null)}>
          <div className="popup-content" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding:'24px 28px 28px' }}>
              ...
              {selectedMeal.calories && <div style={{ fontSize:'12px', color:'var(--orange)', fontWeight:700, marginBottom:'16px' }}>{selectedMeal.calories}</div>}
              ...
              {selectedMeal.allergens && (
                <div style={{ background:'rgba(200,75,15,0.06)', border:'1px solid rgba(200,75,15,0.2)', borderRadius:'12px', padding:'14px 18px', marginBottom:'20px' }}>
