// ════════════════════════════════════════════════════════════
// ADMIN MEALS PAGE — /admin/meals
// Manage all meals for each stage and meal type
// Add/remove meals, upload photos, edit details
// ════════════════════════════════════════════════════════════
...
export default function MealsPage() {
  ...
  async function loadAll() {
    ...
    if (stagesRes.data) {
      ...
      if (stagesRes.data.length > 0) setSelectedStage(stagesRes.data[0].id)
    }
  ...
  async function saveMeal(meal: Meal) {
    ...
    await supabase.from('meals').update({
      ...
      calories: meal.calories, is_active: meal.is_active,
    }).eq('id', meal.id)
    setSaving(null)
    flash('Meal saved ✓')
  }
  ...
  async function addMeal() {
    ...
    const typeMeals = meals.filter(m => m.meal_type_id === selectedType)
    const { data } = await supabase.from('meals').insert({
      meal_type_id: selectedType, stage_id: selectedStage,
      name: 'New Meal', description: 'Meal description here',
      ingredients: '', allergens: '', calories: '',
      position: typeMeals.length + 1,
    }).select().single()
  ...
  const visibleMeals = meals.filter(m => m.meal_type_id === selectedType)

  if (loading) return <div style={{ padding: '40px', color: '#7A7068' }}>Loading meals...</div>
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
        <div>
          ...
          <p style={{ fontSize: '13px', color: '#7A7068' }}>Add meals per stage and category. Upload photos and edit details.</p>
        </div>
      ...
      {/* Stage selector */}
      <div style={{ background: 'white', borderRadius: '12px', padding: '16px', border: '1px solid rgba(0,0,0,0.05)', marginBottom: '16px' }}>
        <label style={{ display: 'block', fontSize: '11px', fontWeight: 600, color: '#C9A98A', marginBottom: '8px' }}>SELECT STAGE</label>
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {stages.map(stage => (
            <button key={stage.id} onClick={() => setSelectedStage(stage.id)} style={{
              padding: '8px 16px', borderRadius: '8px', border: 'none', cursor: 'pointer',
              fontSize: '13px', fontWeight: 500,
              background: selectedStage === stage.id ? '#1C1C1A' : '#F5F3EE',
              color: selectedStage === stage.id ? 'white' : '#7A7068',
            }}>
              {stage.name} <span style={{ opacity: 0.6, fontSize: '11px' }}>({stage.age_range})</span>
            </button>
          ))}
        </div>
      </div>

      {/* Meal type tabs — with editable names */}
      <div style={{ background: 'white', borderRadius: '12px', padding: '16px', border: '1px solid rgba(0,0,0,0.05)', marginBottom: '20px' }}>
        <label style={{ display: 'block', fontSize: '11px', fontWeight: 600, color: '#C9A98A', marginBottom: '8px' }}>MEAL CATEGORIES (tap to switch, edit name inline)</label>
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
          {stageMealTypes.map(mt => (
            <div key={mt.id} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
              <button onClick={() => setSelectedType(mt.id)} style={{
                padding: '8px 14px', borderRadius: '8px', border: '1.5px solid',
                cursor: 'pointer', fontSize: '12px', fontWeight: 600,
                borderColor: selectedType === mt.id ? '#E8834A' : 'rgba(0,0,0,0.1)',
                background: selectedType === mt.id ? '#FFF0EA' : 'white',
                color: selectedType === mt.id ? '#E8834A' : '#7A7068',
              }}>
                {mt.name}
              </button>
              {selectedType === mt.id && (
                <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                  <input
                    ...
                    onBlur={e => saveMealTypeName(mt.id, e.target.value)}
                    style={{ ...inp, width: '120px', fontSize: '12px', padding: '6px 10px' }}
      ...
      <div style={{ fontSize: '12px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#C9A98A', marginBottom: '12px' }}>
        {visibleMeals.length} meals in {stageMealTypes.find(t => t.id === selectedType)?.name || ''}
      </div>

      {/* Meals list */}
      {visibleMeals.sort((a, b) => a.position - b.position).map(meal => (
        <div key={meal.id} style={{ background: 'white', borderRadius: '14px', border: '1px solid rgba(0,0,0,0.06)', marginBottom: '10px', overflow: 'hidden' }}>
          ...
          <div onClick={() => setExpanded(expanded === meal.id ? null : meal.id)}
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: '12px', color: '#7A7068' }}>{meal.calories || 'No calories set'}</div>
            </div>
            <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
              {!meal.is_active && <span style={{ fontSize: '10px', background: '#FEE2E2', color: '#DC2626', padding: '2px 8px', borderRadius: '50px', fontWeight: 600 }}>Hidden</span>}
              ...
              <div style={{ paddingTop: '16px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
                  <div>
                    ...
                    <input value={meal.name} onChange={e => setMeals(meals.map(m => m.id === meal.id ? { ...m, name: e.target.value } : m))} style={inp} />
                  </div>
                  <div>
                    <label style={lbl}>Calories (optional)</label>
                    <input value={meal.calories} onChange={e => setMeals(meals.map(m => m.id === meal.id ? { ...m, calories: e.target.value } : m))} style={inp} placeholder="e.g. 120 kcal" />
                  </div>
                </div>

                <div style={{ marginBottom: '10px' }}>
                  <label style={lbl}>Description</label>
                  <textarea value={meal.description} onChange={e => setMeals(meals.map(m => m.id === meal.id ? { ...m, description: e.target.value } : m))} rows={2} style={{ ...inp, resize: 'vertical' }} />
                </div>

                <div style={{ marginBottom: '10px' }}>
                  <label style={lbl}>Ingredients</label>
                  <textarea value={meal.ingredients} onChange={e => setMeals(meals.map(m => m.id === meal.id ? { ...m, ingredients: e.target.value } : m))} rows={2} style={{ ...inp, resize: 'vertical' }} placeholder="List all ingredients here" />
                </div>

                <div style={{ marginBottom: '14px' }}>
                  <label style={lbl}>Allergens</label>
                  <input value={meal.allergens} onChange={e => setMeals(meals.map(m => m.id === meal.id ? { ...m, allergens: e.target.value } : m))} style={inp} placeholder="e.g. Dairy, Gluten, Nuts" />
                </div>

                <div style={{ display: 'flex', gap: '8px' }}>
                  <button onClick={() => saveMeal(meal)} style={btn('#E8834A')}>{saving === meal.id ? 'Saving...' : 'Save Meal'}</button>
                  <button onClick={() => setMeals(meals.map(m => m.id === meal.id ? { ...m, is_active: !m.is_active } : m))}
                    style={btn(meal.is_active ? '#4A7C59' : '#C9A98A', true)}>
                    {meal.is_active ? '👁 Visible' : '🙈 Hidden'}
                  </button>
      ...
      {visibleMeals.length === 0 && (
        <div style={{ textAlign: 'center', padding: '40px', color: '#C9A98A', background: 'white', borderRadius: '14px', border: '1px solid rgba(0,0,0,0.05)', fontSize: '14px' }}>
          No meals yet. Click "+ Add Meal" to add your first one.
        </div>
      )}
    </div>
  )
