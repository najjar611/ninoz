// ════════════════════════════════════════════════════════════
// ADMIN SCHEDULE PAGE — /admin/stages/[stageId]/[planId]
// Edit weekly schedule for a specific plan
// Add/remove days (5, 6, or 7)
// Edit meal names per day
// Upload meal photo per day
// ════════════════════════════════════════════════════════════

'use client'

import { useState, useEffect, useRef } from 'react'
import { createClient } from '@/lib/supabase/client'
import { useParams } from 'next/navigation'
import Link from 'next/link'

type Stage = { id: string; name: string }
type Plan = { id: string; name: string; price: string }
type DaySchedule = {
  id: string; plan_id: string; day: string; day_number: number
  breakfast: string; lunch: string; dinner: string; image_url?: string | null
}

const DAY_NAMES = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday']
const MEAL_COLORS = { breakfast: '#E8834A', lunch: '#4A7C59', dinner: '#7B5EA7' }
const MEAL_BG = { breakfast: '#FFF0EA', lunch: '#E8F5EE', dinner: '#F0ECF9' }

export default function PlanSchedulePage() {
  const supabase = createClient()
  const { stageId, planId } = useParams<{ stageId: string; planId: string }>()
  const fileRefs = useRef<Record<string, HTMLInputElement | null>>({})

  const [stage, setStage] = useState<Stage | null>(null)
  const [plan, setPlan] = useState<Plan | null>(null)
  const [schedule, setSchedule] = useState<DaySchedule[]>([])
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState<string | null>(null)
  const [savingAll, setSavingAll] = useState(false)
  const [uploading, setUploading] = useState<string | null>(null)
  const [msg, setMsg] = useState({ text: '', type: 'success' })

  useEffect(() => { load() }, [stageId, planId])

  async function load() {
    setLoading(true)
    const [stageRes, planRes, schedRes] = await Promise.all([
      supabase.from('stages').select('id, name').eq('id', stageId).single(),
      supabase.from('plan_details').select('id, name, price').eq('id', planId).single(),
      supabase.from('weekly_schedules').select('*').eq('plan_id', planId).order('day_number'),
    ])
    if (stageRes.data) setStage(stageRes.data)
    if (planRes.data) setPlan(planRes.data)
    if (schedRes.data) setSchedule(schedRes.data)
    setLoading(false)
  }

  function flash(text: string, type = 'success') {
    setMsg({ text, type })
    setTimeout(() => setMsg({ text: '', type: 'success' }), 3000)
  }

  // ── SAVE ONE DAY ──
  async function saveDay(day: DaySchedule) {
    setSaving(day.id)
    await supabase.from('weekly_schedules').update({
      day: day.day, breakfast: day.breakfast,
      lunch: day.lunch, dinner: day.dinner,
    }).eq('id', day.id)
    setSaving(null)
    flash('Day saved ✓')
  }

  // ── SAVE ALL DAYS ──
  async function saveAll() {
    setSavingAll(true)
    for (const day of schedule) {
      await supabase.from('weekly_schedules').update({
        day: day.day, breakfast: day.breakfast,
        lunch: day.lunch, dinner: day.dinner,
      }).eq('id', day.id)
    }
    setSavingAll(false)
    flash('All days saved ✓')
  }

  // ── ADD DAY ──
  async function addDay() {
    if (schedule.length >= 7) { flash('Maximum 7 days', 'error'); return }
    const nextNum = schedule.length + 1
    const dayName = DAY_NAMES[nextNum - 1] || `Day ${nextNum}`
    const { data } = await supabase.from('weekly_schedules').insert({
      plan_id: planId, day: dayName, day_number: nextNum,
      breakfast: 'Meal Name', lunch: 'Meal Name', dinner: 'Meal Name',
    }).select().single()
    if (data) setSchedule([...schedule, data])
    flash('Day added ✓')
  }

  // ── REMOVE LAST DAY ──
  async function removeLastDay() {
    if (schedule.length <= 1) { flash('Must have at least 1 day', 'error'); return }
    if (!confirm('Remove the last day?')) return
    const last = schedule[schedule.length - 1]
    await supabase.from('weekly_schedules').delete().eq('id', last.id)
    setSchedule(schedule.slice(0, -1))
    flash('Day removed')
  }

  // ── UPLOAD MEAL PHOTO ──
  async function uploadDayImage(dayId: string, file: File) {
    if (!file.type.startsWith('image/')) { flash('Please upload an image', 'error'); return }
    if (file.size > 5 * 1024 * 1024) { flash('Image must be under 5MB', 'error'); return }
    setUploading(dayId)
    const ext = file.name.split('.').pop()
    const fileName = `meal-${dayId}.${ext}`
    const { error } = await supabase.storage.from('meals').upload(fileName, file, { upsert: true })
    if (error) { flash(`Upload failed: ${error.message}`, 'error'); setUploading(null); return }
    const { data: { publicUrl } } = supabase.storage.from('meals').getPublicUrl(fileName)
    setSchedule(schedule.map(d => d.id === dayId ? { ...d, image_url: publicUrl } : d))
    setUploading(null)
    flash('Photo uploaded ✓')
  }

  const inp = { width: '100%', padding: '8px 10px', borderRadius: '7px', border: '1px solid #E8D5C4', fontSize: '12px', outline: 'none', boxSizing: 'border-box' as const }
  const btn = (color: string, small = false) => ({ padding: small ? '5px 10px' : '8px 16px', borderRadius: '7px', border: 'none', background: color, color: 'white', cursor: 'pointer', fontSize: small ? '11px' : '12px', fontWeight: 600 as const })

  if (loading) return <div style={{ padding: '40px', color: '#7A7068' }}>Loading schedule...</div>

  return (
    <div>
      {/* Breadcrumb */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', color: '#7A7068', marginBottom: '20px', flexWrap: 'wrap' }}>
        <Link href="/admin/stages" style={{ color: '#E8834A', textDecoration: 'none' }}>Stages</Link>
        <span>→</span>
        <Link href={`/admin/stages/${stageId}`} style={{ color: '#E8834A', textDecoration: 'none' }}>{stage?.name}</Link>
        <span>→</span>
        <span style={{ color: '#1C1C1A', fontWeight: 500 }}>{plan?.name}</span>
      </div>

      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: 700, color: '#1C1C1A', marginBottom: '4px' }}>
            {plan?.name} — Schedule
          </h1>
          <p style={{ fontSize: '13px', color: '#7A7068' }}>{schedule.length} days · {plan?.price}</p>
        </div>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
          {msg.text && <div style={{ background: msg.type === 'success' ? '#E8F5EE' : '#FEE2E2', color: msg.type === 'success' ? '#2D6A4F' : '#DC2626', padding: '8px 16px', borderRadius: '8px', fontSize: '13px', fontWeight: 500 }}>{msg.text}</div>}
          <button onClick={removeLastDay} disabled={schedule.length <= 1} style={btn('#7A7068', true)}>− Remove Day</button>
          <button onClick={addDay} disabled={schedule.length >= 7} style={btn('#4A7C59', true)}>+ Add Day</button>
          <button onClick={saveAll} disabled={savingAll} style={btn('#E8834A')}>
            {savingAll ? 'Saving...' : '💾 Save All'}
          </button>
        </div>
      </div>

      {/* Day cards grid */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '12px' }}>
        {schedule.map(day => (
          <div key={day.id} style={{ background: 'white', borderRadius: '14px', padding: '16px', border: '1px solid rgba(0,0,0,0.05)' }}>

            {/* Day header */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <div style={{ width: '26px', height: '26px', borderRadius: '50%', background: '#FFF0EA', color: '#E8834A', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 700, flexShrink: 0 }}>
                  {day.day_number}
                </div>
                {/* Day name is editable */}
                <input
                  value={day.day}
                  onChange={e => setSchedule(schedule.map(d => d.id === day.id ? { ...d, day: e.target.value } : d))}
                  style={{ fontWeight: 700, fontSize: '14px', color: '#1C1C1A', border: 'none', outline: 'none', background: 'transparent', width: '110px' }}
                />
              </div>
              {/* Photo upload */}
              <div>
                <input type="file" accept="image/*" ref={el => { fileRefs.current[day.id] = el }}
                  onChange={e => { const f = e.target.files?.[0]; if (f) uploadDayImage(day.id, f) }}
                  style={{ display: 'none' }} />
                <button onClick={() => fileRefs.current[day.id]?.click()}
                  style={{ ...btn(uploading === day.id ? '#C9A98A' : '#7B5EA7', true), display: 'flex', alignItems: 'center', gap: '3px' }}
                  disabled={uploading === day.id}>
                  {day.image_url ? '🔄' : '📤'} {uploading === day.id ? '...' : 'Photo'}
                </button>
              </div>
            </div>

            {/* Meal photo preview */}
            {day.image_url && (
              <img src={day.image_url} alt={day.day} style={{ width: '100%', height: '80px', objectFit: 'cover', borderRadius: '8px', marginBottom: '10px' }} />
            )}

            {/* Breakfast / Lunch / Dinner */}
            {(['breakfast','lunch','dinner'] as const).map(meal => (
              <div key={meal} style={{ marginBottom: '8px' }}>
                <div style={{ display: 'inline-block', fontSize: '10px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', padding: '2px 8px', borderRadius: '50px', background: MEAL_BG[meal], color: MEAL_COLORS[meal], marginBottom: '3px' }}>
                  {meal}
                </div>
                <input
                  value={day[meal]}
                  onChange={e => setSchedule(schedule.map(d => d.id === day.id ? { ...d, [meal]: e.target.value } : d))}
                  style={inp}
                  placeholder={`${meal} meal name`}
                />
              </div>
            ))}

            <button onClick={() => saveDay(day)} style={{ ...btn('#E8834A'), width: '100%', marginTop: '6px' }}>
              {saving === day.id ? 'Saving...' : 'Save Day'}
            </button>
          </div>
        ))}
      </div>

      {schedule.length === 0 && (
        <div style={{ textAlign: 'center', padding: '48px', color: '#C9A98A', background: 'white', borderRadius: '16px', border: '1px solid rgba(0,0,0,0.05)' }}>
          No days yet. Click "+ Add Day" to start.
        </div>
      )}
    </div>
  )
}
