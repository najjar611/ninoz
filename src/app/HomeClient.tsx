...
type Stage = { id: string; name: string; age_range: string; description: string; image_url: string | null; emoji: string; tag: string | null; tag_color: string; card_bg: string; card_border: string; plan_details?: { id: string; name: string; price: string; is_active: boolean }[] }
  ...
  return (
    <>
      <style>{`
        ...
        .nav-link { font-size:14px; color:white; background:none; border:none; cursor:pointer; font-weight:600; position:relative; padding-bottom:2px; transition:opacity 0.2s; text-shadow:0 1px 4px rgba(0,0,0,0.3); }
      ...
      <main style={{ background:'var(--cream)' }}>
        ...
        <section style={{ position:'relative', minHeight:'100vh', display:'flex', flexDirection:'column' }}>
          ...
          <nav style={{ position:'fixed', top:0, left:0, right:0, zIndex:100, background:'rgba(0,0,0,0.2)', backdropFilter:'blur(8px)', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 6%', height:'68px' }}>
            ...
            <div className="nav-links-wrap" style={{ display:'flex', gap:'36px' }}>
              {[{ label:'Menu', id:'offers' },{ label:'How It Works', id:'how' },{ label:'FAQ', id:'faq' },{ label:'About', id:'why' }].map(item => (
          ...
          <div className="hero-content" style={{ position:'relative', zIndex:1, flex:1, display:'flex', flexDirection:'column', justifyContent:'center', padding:'120px 6% 80px', maxWidth:'700px' }}>
            ...
            <div className="hero-badges" style={{ display:'flex', gap:'24px', marginBottom:'40px', flexWrap:'wrap' }}>
              {[{ icon:'🌿', label:'Fresh Ingredients' },{ icon:'🚫', label:'No Preservatives' },{ icon:'⏰', label:'Cooked Daily' },{ icon:'👨‍⚕️', label:'Pediatrician Approved' }].map(badge => (
        ...
        <section id="how" style={{ background:'var(--cream-dark)', padding:'80px 6%' }}>
          ...
          <div className="how-wrap" style={{ display:'flex', gap:'20px', maxWidth:'1100px', margin:'0 auto 40px' }}>
            {howSteps.map((step, i) => (
              <div key={step.id} className={`h-item step-card reveal delay-${Math.min(i+1,3)}`} style={{ padding:'32px 28px', flex:1 }}>
        ...
        <section id="why" style={{ padding:'80px 6%', background:'var(--cream)' }}>
          <div className="why-grid" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'80px', alignItems:'center', maxWidth:'1200px', margin:'0 auto 60px' }}>
            ...
            <div className="reveal delay-1" style={{ height:'480px', borderRadius:'24px', overflow:'hidden', background:'#F2EAE0', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'80px' }}>
          ...
          {testimonials.length > 0 ? (
            <div className="testi-wrap" style={{ display:'flex', gap:'16px', maxWidth:'1100px', margin:'0 auto' }}>
              {testimonials.map((t, i) => (
                <div key={t.id} className={`h-item testi-card reveal delay-${Math.min(i+1,3)}`} style={{ padding:'26px 24px', flex:1 }}>
          ...
          <section id="blog" style={{ padding:'80px 6%', background:'var(--cream)' }}>
            ...
            <div className="blog-grid" style={{ display:'grid', gap:'20px', maxWidth:'1100px', margin:'0 auto' }}>
              {blogPosts.map((post, i) => (
                <a key={post.id} href="#" className={`blog-card reveal delay-${Math.min(i+1,3)}`}>
            ...
            <div className="contact-carousel reveal delay-1">
              <div className="contact-slide">
                <a href={`https://wa.me/${settings.whatsapp_number||'966XXXXXXXXX'}`} target="_blank" className="contact-card cc-wa">
                  <div style={{ width:'64px', height:'64px', borderRadius:'18px', background:'rgba(37,211,102,0.12)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                  ...
                  <div style={{ fontSize:'18px', fontWeight:700, color:'white' }}>WhatsApp Us</div>
                  <div style={{ fontSize:'14px', color:'rgba(255,255,255,0.5)' }}>Fastest response — chat directly with our team</div>
                  <div style={{ background:'rgba(37,211,102,0.15)', color:'#25D366', fontSize:'13px', fontWeight:700, padding:'8px 22px', borderRadius:'50px' }}>Message us →</div>
                </a>
              </div>
              <div className="contact-slide">
                <a href={`https://instagram.com/${settings.instagram_handle||'ninoz.sa'}`} target="_blank" className="contact-card cc-ig">
                  ...
                  <div style={{ fontSize:'18px', fontWeight:700, color:'white' }}>Follow on Instagram</div>
                  <div style={{ fontSize:'14px', color:'rgba(255,255,255,0.5)' }}>Daily content & behind the scenes</div>
                  <div style={{ background:'rgba(225,48,108,0.15)', color:'#E1306C', fontSize:'13px', fontWeight:700, padding:'8px 22px', borderRadius:'50px' }}>@{settings.instagram_handle||'ninoz.sa'} →</div>
                </a>
              </div>
            </div>
          </div>
